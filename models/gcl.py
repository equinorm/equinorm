...
class E_GCL(nn.Module):
    ...
    def edge_model(self, source, target, radial, edge_attr):
        '''
        source: [B*num_edges, 64]
        target: [B*num_edges, 64]
        radial: [B*num_edges, 1]
        edge_attr: [B*num_edges,2]
        '''
    ...
    def coord_model(self, coord, edge_index, coord_diff, edge_feat):
        row, col = edge_index
        trans = coord_diff * self.coord_mlp(edge_feat) # [num_edges, 3]
        trans = torch.clamp(trans, min=-100, max=100) #This is never activated but just in case it case it explosed it may save the train
        agg = unsorted_segment_mean(trans, row, num_segments=coord.size(0))
        coord += agg*self.coords_weight # [B*N, 3]
        return coord
...
def unsorted_segment_mean(data, segment_ids, num_segments):
    '''
    data: [num_edges, 3]
    segment_ids: [num_edges], [0,0,0,1,1,...]
    num_segments: B*N
    '''
    result_shape = (num_segments, data.size(1)) # [B*N, 3]
    segment_ids = segment_ids.unsqueeze(-1).expand(-1, data.size(1)) # [num_edges, 3]
    result = data.new_full(result_shape, 0)  # Init empty result tensor
    count = data.new_full(result_shape, 0)  # Init empty count tensor
    result.scatter_add_(0, segment_ids, data)
    count.scatter_add_(0, segment_ids, torch.ones_like(data))
    return result / count.clamp(min=1) # mean